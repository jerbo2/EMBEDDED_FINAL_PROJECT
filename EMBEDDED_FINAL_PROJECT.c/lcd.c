...
const uint16_t ports[] = { (uint16_t) &P1OUT, (uint16_t) &P2OUT, (uint16_t) &P3OUT, (uint16_t) &P4OUT};
const uint16_t dirs[] = { (uint16_t) &P1DIR, (uint16_t) &P2DIR, (uint16_t) &P3DIR, (uint16_t) &P4DIR};
...
// Function to write 4 bits of data to D4-D7 pins
...
void lcd_init(uint8_t dat_port, uint8_t d4, uint8_t d5, uint8_t d6, uint8_t d7, uint8_t rs_port, uint8_t rs, uint8_t en_port, uint8_t en)
{
	...
	if(dat_port == 2)
		P2DIR &= ~(lcdPins[0] + lcdPins[1] + lcdPins[2] + lcdPins[3]);
	if(rs_port == 2)
		P2DIR &= ~rsPin;
	if(en_port == 2)
		P2DIR &= ~enPin;
